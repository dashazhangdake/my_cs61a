...
def hefty_hogs(player_score, opponent_score):
    ...
    if opponent_score == 0:
        ...
        while opponent_score != 0:
            curr_fid = opponent_score % 10
            opponent_score = opponent_score // 10
            picked_fn = digit_fn(curr_fid)
            res = picked_fn(res)
    return res % 30
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    def player_turn(strategy, scorea, scoreb, dice_p, goal_p):
        score_this_turn = take_turn(strategy(scorea, scoreb), scorea, scoreb, dice_p, goal_p)
        return score_this_turn
    while (score0 < goal) and (score1 < goal):
        if who == 0:
            score0 = score0 + player_turn(strategy0, score0, score1, dice, goal)
            score0 += hog_pile(score0, score1)
            # print('p0', score0)
        elif who == 1:
            score1 = score1 + player_turn(strategy1, score1, score0, dice, goal)
            score1 += hog_pile(score1, score0)
            # print('p1', score1)
        # score0 = score0 + take_turn(strategy0(score0, score1), score0, score1, dice, goal)
        # score0 = score1 + hog_pile(score0, score1)
        # else:
        #     raise ValueError('invalid player')
        # print(say(score0, score1, who))
...
def announce_lead_changes(score0, score1, last_leader=None):
    ...
    if leader != last_leader and leader != None:
        message = "Player %d takes the lead by %d" % (leader, abs(score0 - score1))
    else:
        message = None
...
def make_averaged(original_function, total_samples=1000):
    ...
    # BEGIN PROBLEM 8
    "*** YOUR CODE HERE ***"
    def orginnal_fn_caller(*args):
        res = 0
        sample_cnt = 0
        while sample_cnt < total_samples:
            rolled_sum = original_function(*args)
            res = res + rolled_sum
            sample_cnt += 1
        return res / total_samples
    return orginnal_fn_caller
    # END PROBLEM 8
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    candidate_score = 0
    for i in range(1, 11):
        roll_score = make_averaged(roll_dice, total_samples)(i, dice)
        if candidate_score < roll_score:
            roll_candidate = i
            candidate_score = roll_score
...
def hog_pile_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    base_score_after_roll0 = score + hefty_hogs(score, opponent_score) # when roll 0, you are triggering hefty and hog_pile
    if hog_pile(base_score_after_roll0, opponent_score) != 0:
        return 0
    else:
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    """
    def regular_throw(s0, s1):
        if s0 == 0 and opponent_score == 0:
            return 6
        else:
            return hog_pile_strategy(s0, s1, threshold=8, num_rolls=6)

    def roll_strategy():
        return regular_throw(score, opponent_score)
    # BEGIN PROBLEM 12
    return roll_strategy()  # Remove this line once implemented.
    # END PROBLEM 12
