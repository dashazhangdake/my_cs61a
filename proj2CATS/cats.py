...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    # BEGIN PROBLEM 5
    "*** YOUR CODE HERE ***"
    if typed_word in word_list: # Special case, typed_word has a perfect match
        return typed_word

    diff_value = [diff_function(typed_word, wd, limit) for wd in word_list]
    min_diff_idx = diff_value.index(min(diff_value)) # Return the first index of first occurance
    if diff_value[min_diff_idx] > limit:
        return typed_word
    else:
        return word_list[min_diff_idx]
    # END PROBLEM 5
...
def sphinx_swaps(start, goal, limit):
    ...
    if start == '':
        ...
        if start[0] != goal[0]:
            return change + 1 + sphinx_swaps(start[1:], goal[1:], limit - 1)
        else:
...
def report_progress(sofar, prompt, user_id, upload):
    ...
    # BEGIN PROBLEM 8
    "*** YOUR CODE HERE ***"
    player_dict = {'id':'', 'progress':0}
    correct = 0
    for i, wd in enumerate(sofar):
        if wd == prompt[i]:
            correct += 1
        else:
            break
    progress = correct / len(prompt)
    player_dict['id'] = user_id
    player_dict['progress'] = progress
    upload(player_dict)
    return progress
    # END PROBLEM 8
...
def time_per_word(words, times_per_player):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    def timestamps(lst_t1p):
        return [(lst_t1p[i] - lst_t1p[i-1]) for i in range(1, len(lst_t1p))]

    res_dict = {'words': words, 'times': []}
    for e in times_per_player:
        res_dict['times'].append(timestamps(e))
    return res_dict
    # END PROBLEM 9
